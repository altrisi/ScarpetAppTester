...
public abstract class ScarpetEvents extends Event {
	...
	public static final ScarpetEvents PRE_TEST_STARTED = new ScarpetEvents("pre_test_started", 2, false) {
		public void dispatch(Test test) {
			this.handler.call(() -> {
				return Arrays.asList(new StringValue(test.getAppName()), new StringValue(test.getTestName()));
			}, SOURCE_SUPPLIER);
		}
	};
	...
	public static final ScarpetEvents TEST_FINISHED = new ScarpetEvents("test_finished", 3, false) {
		public void dispatch(Test test) {
			this.handler.call(() -> {
				return Arrays.asList(new StringValue(test.getAppName()), new StringValue(test.getTestName()), new NumericValue(test.successfulSoFar()));
			}, SOURCE_SUPPLIER);
		}
	};
